...
def start_tango_server():
    ...
    def server_worker():
        ...
        # CREATE & SET EVENT LOOP FOR THIS THREAD BEFORE TANGO INIT
        global global_event_loop
        global_event_loop = asyncio.new_event_loop()
        asyncio.set_event_loop(global_event_loop)
        print("🌐 Event loop initialized for Tango server thread")
        logger.info("🌐 Event loop initialized for Tango server thread")
        try:
