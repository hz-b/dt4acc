...
class TestOrbitData:
    ...
    def wait_for_heartbeat(self, settled):
        """Ensure heartbeat has fired before reading."""
        pass

    def test_orbit_x_is_not_all_zeros(self, dp_twiss_orbit):
    ...
    def test_orbit_y_is_finite(self, dp_twiss_orbit):
        arr = _arr(dp_twiss_orbit, "orbit_y")
        assert np.all(np.isfinite(arr)), "orbit_y contains NaN or Inf"

    def test_orbit_x_y_same_length(self, dp_twiss_orbit):
        x = _arr(dp_twiss_orbit, "orbit_x")
        y = _arr(dp_twiss_orbit, "orbit_y")
        assert len(x) == len(y), \
...
class TestTwissData:
    ...
    def test_beta_x_is_positive(self, dp_twiss_orbit):
        arr = _arr(dp_twiss_orbit, "beta_x")
        assert len(arr) >= 10, "beta_x array too short"
        assert np.all(arr > 0), \
            f"beta_x has non-positive values (min={arr.min():.4f})"

    def test_beta_y_is_positive(self, dp_twiss_orbit):
        arr = _arr(dp_twiss_orbit, "beta_y")
        assert len(arr) >= 10, "beta_y array too short"
        assert np.all(arr > 0), \
            f"beta_y has non-positive values (min={arr.min():.4f})"

    def test_alpha_x_is_finite(self, dp_twiss_orbit):
        arr = _arr(dp_twiss_orbit, "alpha_x")
        assert np.all(np.isfinite(arr)), "alpha_x contains NaN or Inf"

    def test_alpha_y_is_finite(self, dp_twiss_orbit):
        arr = _arr(dp_twiss_orbit, "alpha_y")
        assert np.all(np.isfinite(arr)), "alpha_y contains NaN or Inf"

    def test_nu_x_is_monotonically_increasing(self, dp_twiss_orbit):
        arr = _arr(dp_twiss_orbit, "nu_x")
        assert len(arr) >= 10
        diffs = np.diff(arr)
        assert np.all(diffs >= 0), \
            f"nu_x not monotonically increasing — max decrease: {diffs.min():.6f}"

    def test_nu_y_is_monotonically_increasing(self, dp_twiss_orbit):
        arr = _arr(dp_twiss_orbit, "nu_y")
        assert len(arr) >= 10
        diffs = np.diff(arr)
        assert np.all(diffs >= 0), \
            f"nu_y not monotonically increasing — max decrease: {diffs.min():.6f}"

    def test_all_twiss_arrays_same_length(self, dp_twiss_orbit):
        lengths = {
            attr: len(_arr(dp_twiss_orbit, attr))
            for attr in ("beta_x", "beta_y", "alpha_x", "alpha_y", "nu_x", "nu_y")
        }
        unique = set(lengths.values())
        assert len(unique) == 1, f"Twiss array length mismatch: {lengths}"

    def test_twiss_and_orbit_same_length(self, dp_twiss_orbit):
        orbit_len = len(_arr(dp_twiss_orbit, "orbit_x"))
        twiss_len = len(_arr(dp_twiss_orbit, "beta_x"))
...
class TestBPMData:
    ...
    def test_bpm_names_not_empty(self, dp_bpm):
        names = dp_bpm.read_attribute("bpm_names_attr").value
        assert names is not None and len(names) > 0, \
            "bpm_names_attr is empty — orbit not pushed to BPMManagerDevice"

    def test_bpm_names_are_bpm_elements(self, dp_bpm):
        names = dp_bpm.read_attribute("bpm_names_attr").value
    ...
    def test_bpm_x_is_finite(self, dp_bpm):
        arr = _arr(dp_bpm, "bpm_x_attr")
        assert len(arr) > 0, "bpm_x_attr is empty"
        assert np.all(np.isfinite(arr)), "bpm_x_attr contains NaN or Inf"

    def test_bpm_y_is_finite(self, dp_bpm):
        arr = _arr(dp_bpm, "bpm_y_attr")
        assert len(arr) > 0, "bpm_y_attr is empty"
        assert np.all(np.isfinite(arr)), "bpm_y_attr contains NaN or Inf"

    def test_bpm_arrays_consistent_length(self, dp_bpm):
        x     = _arr(dp_bpm, "bpm_x_attr")
        y     = _arr(dp_bpm, "bpm_y_attr")
        names = dp_bpm.read_attribute("bpm_names_attr").value
...
class TestTuneData:
    ...
    def test_tune_hor_is_nonzero(self, dp_tune):
        val = float(dp_tune.read_attribute("hor").value)
        assert val != 0.0, "tune.hor is zero — tune not pushed to TuneDevice"

    def test_tune_vert_is_nonzero(self, dp_tune):
        val = float(dp_tune.read_attribute("vert").value)
        assert val != 0.0, "tune.vert is zero — tune not pushed to TuneDevice"

    def test_tune_hor_is_finite(self, dp_tune):
        val = float(dp_tune.read_attribute("hor").value)
        assert np.isfinite(val), f"tune.hor is not finite: {val}"

    def test_tune_vert_is_finite(self, dp_tune):
        val = float(dp_tune.read_attribute("vert").value)
        assert np.isfinite(val), f"tune.vert is not finite: {val}"

    def test_tune_fractional_part_in_range(self, dp_tune):
        """Fractional part of tune must be in (0, 1)."""
        hor  = float(dp_tune.read_attribute("hor").value) % 1.0
        vert = float(dp_tune.read_attribute("vert").value) % 1.0
