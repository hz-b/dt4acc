"""
Todo:
    need to check how the offset of BPM's is changed when
    do I need to use the Loco model

Differences currently produced go back to this inconsistency
Cross checks per hand produced the exactly same numbers
"""
...
expected_families =  ["BPMx", "BPMy"]


def _family_file(family_name: str) -> Path:
    func_name = "hw2physics"
...
def test_hw2physics_matches_matlab_reference(reference_data: ReferenceCurvesForFamily):
    for reference_curve in reference_data.curves:
        # Todo: track down the correct translation object
        device = reference_curve.device_id
        dev_index = [device.sector, device.child]

        convs = ts.objects_for_device(device)
        conv_id, = convs.keys()
        to = ts.get(conv_id)
        vals = [float(to.inverse(point.indep)) for point in reference_curve.curve]
        # Difference produced for supporting debugging
        # Currently there is (only) an offset difference
        # It gets easy to track down with this difference
        tmp = np.array([(point.dep * 1000, val * 1000) for point, val in zip(reference_curve.curve, vals)])
        for point, actual in zip(reference_curve.curve, vals):
            try:
